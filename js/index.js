$(document).ready(() => {
    ...
    const doSearch = () => {
        // Get text from input search box
        ...
        // on success we want to populate the div .video-play with an iframe that will hold the first video result. The class embed-responsive-item is bootstrap made.suggest-list
        // then we call populateSuggestions and pass in the rest of the videos for the suggested video section
        ...
        $.ajax({
            ...
            success: (result) => {
                $('.video-play').text('');
                $('.video-play').append(`<iframe class="embed-responsive-item" src=https://www.youtube.com/embed/${result.items[0].id.videoId} alloFullScrreen title='youtube player' />`);
    ...
    const populateSuggestions = (videos) => {
        ...
        for (video of videos) {
            ...
            let videoElement = `<a href="#" class="suggested" data-videoId=${video.id.videoId} ><img src=${video.snippet.thumnails.medium.url} /></a>`;
